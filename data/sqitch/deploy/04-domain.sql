...
CREATE DOMAIN "dom_image" AS TEXT
    CHECK(
        VALUE ~ '^(https:\/\/|http:\/\/)[\w\\.\/%:?#\[\]@!$&''"\(\)*+\-\^\{\}§ÀÂÃÄÅÆÇÈÉÊËÌ_ÍÎÏÐÑÒÓÔÕÖØŒŠþÙÚÛÜÝŸàáâãäåæçèéêëìíîïðñòóôõöøœšÞùúûüýÿ,;=]+$'
);
...
CREATE DOMAIN "dom_volume" AS TEXT
    CHECK(
        VALUE ~ '^[a-zA-Z\-\,&ÀÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØŒŠþÙÚÛÜÝŸàá.âãäåæçèéêëìíîïðñòóôõöøœšÞùúûüýÿé\d ]{0,10}$'
);
