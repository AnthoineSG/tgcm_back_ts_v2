...
export const productSchema = Joi.object({
  ...
  image: Joi.string()
    .pattern(
      new RegExp(
        "^(https:\\/\\/|http:\\/\\/)[\\w\\.\\/%:?#\\[\\]@!$&''(\\)*+\\-\\^\\{\\}§ÀÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØŒŠþÙÚÛÜÝŸàáâãäåæçèéêëìíîïðñòóôõöøœšÞùúûüýÿ,;=]+$"
      )
    )
    .min(2)
    .max(100)
  ...
  barcode: Joi.string().pattern(new RegExp('^[\\d+]{8,13}$')).min(8).max(13),

  expiration_date: Joi.string()
    .pattern(
      new RegExp(
        '^([0][1-9]|[1-2][0-9]|[3][0-1])\\/([0][1-9]|[1][0-2])\\/[2][0][2][2-9]$'
      )
    )
    .min(10)
    .max(15),
  ...
  price_ht: Joi.number().positive().min(0).max(9999).required(),

  price_promo: Joi.number().positive().min(0).max(9999),

  price_kg: Joi.number().positive().min(0).max(9999),

  tva_id: Joi.number().positive().required(),

  brand_id: Joi.number().positive().required(),

  sub_category_id: Joi.number().positive().required(),
}).required();
